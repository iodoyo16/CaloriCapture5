...
def make_reco_ranks(client, dris, keys, weights, food_list, cutoff=100):
    rank_list = []
    infos = db_functions.db_find_many(client, food_list)
    for info in infos:
        for key in keys:
            loss = Loss(dris, keys, weights, info)
        rank_list.append((loss,info))
    return sorted(rank_list, key=lambda x: x[0])[:cutoff]
